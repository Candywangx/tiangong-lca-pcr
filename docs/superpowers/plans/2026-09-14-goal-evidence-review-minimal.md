...
**Status:** 2026-09-14 修订；仅修改本方案，未实施代码、修改运行配置或接管 Goal。本文取代此前“新增稀疏报告本体 v2、每个新 turn 自动升级”的设计。
...
- [ ] 阶段 0：刷新入口与 prepared 部署证据、脏差异、指纹、来源 turn/任务契约，确定保留现有改动的实施基线。
- [ ] 阶段 1a：先加来源归因/缓存/恢复预算与幂等故障测试，再实现共享恢复决策和各入口预算顺序。
- [ ] 阶段 1b：先加返回失败对象不抛错的入池负例，再统一准备与收取的完整性断言；实现依赖感知的结果聚合、成功数据消费者适配和整次预算。
- [ ] 阶段 2：先加契约 2＋草稿 v2＋完整 report v1 的准备/引用/收取/历史复查回归，再精简草稿与扩展 assembler；固定首次采用的版本组合。
- [ ] 更新 Harness 说明。执行下列定向测试及 `npm run validate`，不降低原校验规则。
- [ ] 用已知失败类型构造脱敏回放；旧报告、凭据、事件与计数不改写。比较同门禁下无效修复和人工调度次数，而非通过率放宽。
