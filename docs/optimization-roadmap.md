...
截至 2026-08-12，在完成 Phase 2、CPC `99000` 物理 pilot 和后续审核通过的 material 提升后，仓库包含
2,878 个 PCR 目录：

| 状态 | 数量 | 消费含义 |
| --- | ---: | --- |
| material methodology | 281 | 按各自 lifecycle/readiness 状态进入消费与审核 |
| `scaffold / empty_scaffold` | 2,597 | `unavailable`，只在显式 legacy/all catalog scope 下可见 |

CPC 3.0 的 2,877 个 leaf 仍由 coverage 完整表达：279 mapped、2,598 unmapped、0 unknown。Current mapping
含 279 条 accepted material edge；2,598 个 retired-id alias 保留旧 identity locator，因此 alias 数量比
...
1. 已实现：ADR、迁移计划、确定性 material index 和完整 classification coverage read model；CPC 3.0 当前为 2,877 leaf、279 mapped、2,598 unmapped、0 unknown。
...
3. 已实现：2,598 条 deterministic alias 以 terminal classification coverage locator 保留旧 id；alias-first
...
5. 已实现：mapping v2/current 只允许 accepted positive edge，包含 decision-maker、UTC decision time 与
   durable decision ref；CPC 3.0 现有 279 条 material edge，CPC 2.1 为 empty v2。Coverage runtime 将 leaf、
...
7. 进行中：CPC `99000` pilot 已删除 1 个 legacy 四文件目录；后续 create/recovery workflow 已完成审核
   通过的原位 material 提升。当前为 281 material、279 accepted mapping、2,877 coverage leaf、2,598
   alias；CPC `98000` 与后续 2,597 个 legacy directory 仍需独立审计后分批迁移。
