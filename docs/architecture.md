...
| `classifications/` | 外部分类体系 source、normalized data、mapping 和派生 coverage | `classifications/systems/**`、`classifications/mappings/**`、`classifications/indexes/**` | 不定义 PCR 目录结构，不用外部 code 充当 PCR identity |
...
`mapping: null`、`pcr: null`。旧 scaffold 只能作为 `legacy_scaffold_compatibility` 返回，不是可用方法学。
...
classification code
  -> classifications/indexes/<system>-<version>-coverage.json
  -> accepted classifications/mappings edge, legacy compatibility, or known-unmapped result
...
suggestion 自动升级为 mapping。`resolve` 对 accepted mapping 返回 material PCR，对已知 unmapped leaf
成功返回空 mapping/PCR，对保留的旧 scaffold 返回显式兼容状态。只有 mapped PCR 才继续检查 readiness；
空 scaffold 的 `guidance` 和 validation 会明确失败。validation 命令默认在 error finding 或结果
inconclusive 时退出 2；仅报告但不影响 shell 状态的工作流必须显式使用 `--fail-on never`。
...
| mapping truth | `classifications/mappings/**` | 外部分类 code 到 material PCR id 的 accepted edge；迁移期仍含旧兼容 entry |
...
- `classifications/indexes/**`：由 normalized classification leaf、mapping 与兼容状态生成的完整 coverage read model。
