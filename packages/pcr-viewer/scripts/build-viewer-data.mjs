...
export function buildViewer({
  ...
  try {
    ...
    commitViewerDeployment({ stageDir: tempDir, outDir: resolvedOutDir, failurePhase: deploymentFailurePhase });
...
export function mirrorViewerArtifactStore({ artifactStore, outDir = defaultOutDir, sourceVerifier = null, deploymentFailurePhase = null } = {}) {
  ...
  try {
    ...
    if (
      !readFileSync(path.join(resolvedStore, "active.json")).equals(activeBytes) ||
      !readFileSync(path.join(resolvedStore, "history-head.json")).equals(historyBytes)
    ) {
