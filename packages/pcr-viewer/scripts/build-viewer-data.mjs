...
export function buildViewer({
  ...
  try {
    const source = acceptedIntegrationHead ?? deriveAcceptedIntegrationHead(resolvedRoot);
...
export function mirrorViewerArtifactStore({ artifactStore, outDir = defaultOutDir } = {}) {
  ...
  const active = JSON.parse(readFileSync(path.join(resolvedStore, "active.json"), "utf8"));
  const uiRef = String(active.ui_bundle_ref ?? "");
  if (!/^sha256:[a-f0-9]{64}$/u.test(uiRef)) throw new ViewerBuilderError("VIEWER_MIRROR_INVALID", "Viewer active UI bundle reference is invalid.");
  const uiObject = JSON.parse(readFileSync(path.join(resolvedStore, "objects", `${uiRef.slice(7)}.json`), "utf8"));
  const assetUrl = String(uiObject?.entry?.asset_url ?? "");
  if (!/^ui\/[a-f0-9]{64}\/$/u.test(assetUrl)) throw new ViewerBuilderError("VIEWER_MIRROR_INVALID", "Viewer active UI bundle URL is invalid.");
  const parentDir = path.dirname(resolvedOutDir);
  mkdirSync(parentDir, { recursive: true });
...
function deriveAcceptedIntegrationHead(root) {
  let commit;
  let tree;
  let sourceRef;
  try {
    commit = gitOutput(root, ["rev-parse", "--verify", "HEAD^{commit}"]);
    tree = gitOutput(root, ["rev-parse", "--verify", "HEAD^{tree}"]);
    sourceRef = gitOutput(root, ["symbolic-ref", "-q", "HEAD"]) || "HEAD";
  } catch (error) {
    throw new ViewerBuilderError(
      "VIEWER_ACCEPTED_INTEGRATION_HEAD_REQUIRED",
      "The split Viewer bootstrap requires a selected accepted Git integration head.",
      { cause: error },
    );
  }
  return {
    snapshotId: `viewer-bootstrap-${commit.slice(0, 12)}`,
    goalId: "viewer-bootstrap",
    harnessSnapshotId: `bootstrap-${commit.slice(0, 12)}`,
    sequence: 1,
    sourceRef,
